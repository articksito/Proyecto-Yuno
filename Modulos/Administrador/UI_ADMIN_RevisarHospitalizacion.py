...
class VentanaRevisarHospitalizacion(QMainWindow):
    def __init__(self, nombre_usuario="Admin"):
        ...
        self.setWindowTitle("Sistema Veterinario Yuno - Revisar Hospitalización (Admin)")
    ...
    def navegar(self, categoria, opcion):
        ...
        if categoria == "Hospitalizacion" and opcion == "Visualizar":
             QMessageBox.information(self, "Sistema", "Ya te encuentras en Revisar Hospitalización.")
             return

        try:
            # Ejemplo de navegación a otras ventanas (Descomentar y adaptar según necesidad)
    ...
    # ============================================================
    #  PANEL CENTRAL (REVISAR HOSPITALIZACIÓN)
    # ============================================================
    def setup_content_panel(self):
        ...
        lbl_header = QLabel("Revisar Hospitalización")
        ...
        # IZQUIERDA: Datos Técnicos (fk_consulta)
    ...
    def setup_search_bar(self):
        ...
        lbl_search = QLabel("ID Hospitalización:")
        ...
        btn_search.clicked.connect(self.buscar_hospitalizacion)
    ...
    def setup_info_board(self, parent_layout):
        ...
        self.txt_observaciones.setStyleSheet("border: none; padding: 15px; font-size: 16px; color: #555;")
        self.txt_observaciones.setText("Ingrese un ID para ver los detalles...")

        board_layout.addWidget(header)
        board_layout.addWidget(self.txt_observaciones)
    ...
    def buscar_hospitalizacion(self):
        id_hosp = self.inp_search.text().strip()

        if not id_hosp.isdigit():
            QMessageBox.warning(self, "Error", "El ID debe ser un número.")
            return

        try:
            # Consulta a tabla hospitalizacion
            columnas = ['fk_consulta', 'observaciones']
            
            resultado = self.conexion.consultar_registro(
                tabla='hospitalizacion',
                id_columna='id_hospitalizacion',
                id_valor=id_hosp,
                columnas=columnas
            )

            if resultado:
                self.lbl_consulta.setText(str(resultado[0]))
                self.txt_observaciones.setText(str(resultado[1]))
                
                QMessageBox.information(self, "Encontrado", "Registro de hospitalización cargado.")
            else:
                self.limpiar_datos()
                QMessageBox.warning(self, "No Encontrado", "No existe un registro con ese ID.")
...
if __name__ == "__main__":
    ...
    app.setFont(font)
    window = VentanaRevisarHospitalizacion()
    window.show()
    sys.exit(app.exec())