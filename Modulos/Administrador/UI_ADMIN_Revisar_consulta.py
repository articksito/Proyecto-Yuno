...
class VentanaRevisarConsulta(QMainWindow):
    def __init__(self, nombre_usuario="Admin"):
        ...
        self.setStyleSheet("""
            ...
            QTableWidget { background-color: white; border: 1px solid #E0E0E0; border-radius: 15px; gridline-color: transparent; font-size: 14px; selection-background-color: #E1BEE7; selection-color: #333; alternate-background-color: #FAFAFA; outline: 0; }
            QHeaderView::section { background-color: #7CEBFC; color: #444; font-weight: bold; border: none; padding: 12px; font-size: 15px; font-family: 'Segoe UI'; }
            QHeaderView::section:first { border-top-left-radius: 15px; }
            QHeaderView::section:last { border-top-right-radius: 15px; }
            QScrollBar:vertical { border: none; background: #F5F5F5; width: 10px; border-radius: 5px; }
        ...
        self.txt_buscar.setStyleSheet("QLineEdit { border: 2px solid #ddd; border-radius: 10px; padding: 5px 10px; font-size: 14px; } QLineEdit:focus { border: 2px solid #7CEBFC; }")
        ...
        btn_buscar.setStyleSheet("QPushButton { background-color: #E1BEE7; color: #4A148C; border-radius: 10px; font-weight: bold; border: none; } QPushButton:hover { background-color: #D1C4E9; }")
        btn_buscar.clicked.connect(self.realizar_busqueda)

        btn_back = QPushButton("↶ Volver")
        btn_back.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_back.setStyleSheet("QPushButton { background-color: #F0F0F0; color: #555; border-radius: 20px; padding: 10px 20px; font-size: 16px; font-weight: bold; border: none; } QPushButton:hover { background-color: #E0E0E0; color: #333; }")
        btn_back.clicked.connect(self.volver_al_menu)

        btn_refresh = QPushButton("↻ Actualizar")
        btn_refresh.setCursor(Qt.CursorShape.PointingHandCursor)
        btn_refresh.setFixedSize(120, 40)
        btn_refresh.setStyleSheet("QPushButton { background-color: #7CEBFC; color: #444; border-radius: 10px; font-weight: bold; border: 1px solid #5CD0E3; } QPushButton:hover { background-color: #5CD0E3; }")
    ...
    def navegar(self, categoria, opcion):
        if categoria == "Consulta" and opcion == "Visualizar": return 
        try:
    ...
    def setup_table(self):
        # 6 Columnas: ID, Fecha, Hora, Mascota, VETERINARIO, Motivo
    ...
    def cargar_datos_tabla(self, filtro=""):
        """Obtiene datos de la BD usando LISTA DE JOINS"""
        ...
        try:
            ...
            orden_por = ('consulta.fecha', 'consulta.hora')
