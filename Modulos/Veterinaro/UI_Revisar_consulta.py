...
class VentanaRevisarConsulta(QMainWindow):
    ...
    def navegar(self, categoria, opcion):
        if categoria == "Consultas" and opcion == "Ver Registro":
    ...
    def setup_white_panel(self):
        ...
        lbl_header = QLabel("Historial Médico")
    ...
    def setup_table(self):
        # Columnas: ID, Fecha, Hora, Consultorio, Pago, Mascota, Veterinario, Motivo
        self.table = QTableWidget()
        self.table.setColumnCount(8)
        self.table.setHorizontalHeaderLabels(["ID", "Fecha", "Hora", "Consul.", "Pago", "Mascota", "Veterinario", "Motivo"])
        ...
        header.setSectionResizeMode(3, QHeaderView.ResizeMode.Fixed) # Consultorio
        self.table.setColumnWidth(3, 80)
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.Fixed) # Pago
        self.table.setColumnWidth(4, 100)
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents) # Mascota
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.ResizeToContents) # Vet
    ...
    def cargar_datos_tabla(self, filtro=""):
        ...
        try:
            ...
            columnas = (
                ...
                'mascota.nombre', 
                'usuario.nombre', # Nombre del Vet
                'consulta.motivo'
            )
            
            orden_por = ('consulta.fecha', 'consulta.hora')
            
            # Joins: Consulta -> Mascota | Consulta -> Veterinario -> Usuario
            mis_joins = [
                ('mascota', 'consulta'),
            ...
            for row_idx, row_data in enumerate(datos):
                ...
                # row_data = (id, fecha, hora, consultorio, pago, mascota, vet_nombre, motivo)
                ...
                item_cons = QTableWidgetItem(str(row_data[3]))
                item_cons.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.table.setItem(row_idx, 3, item_cons)
                
                item_pago = QTableWidgetItem(str(row_data[4]))
                item_pago.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.table.setItem(row_idx, 4, item_pago)
                
                item_mascota = QTableWidgetItem(str(row_data[5]))
                item_mascota.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.table.setItem(row_idx, 5, item_mascota)
                
                item_vet = QTableWidgetItem(f"Dr. {row_data[6]}")
                item_vet.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                self.table.setItem(row_idx, 6, item_vet)
                
                # Motivo con Tooltip
                motivo_texto = str(row_data[7])
                item_motivo = QTableWidgetItem(motivo_texto)
                item_motivo.setToolTip(motivo_texto)
                self.table.setItem(row_idx, 7, item_motivo)
